...
# Date: 11/06/2019
# Description: Curation of multiple sequencing studdies in AML into one dataframe for meta analysis of mutational and clinical correlations
# ==========================================================================================================================================
...
variants <- TCGAL_variants_raw %>%
  select("UPN", "TCGA_id", "gene_name", "type", "trv_type", "TumorVAF", "amino_acid_change")
...
mut_table_pts <- select(mut_table_pts, "value", "count.UPN")
...
# samples need exome sequencing performed then, the first sample per patients (preferably bone marrow) is selected for sample analysis
pt_subset_2 <- subset(BeatAML_sample_data_types, BeatAML_sample_data_types$exomeSeq == "y")


# create a new column to annotate the type of sample and then loop through the clinical annotations and decide if the sample is de novo, secondary, or relapse
...
pt_subset_2 <- select(pt_subset_2, LabId, Subset, PatientId, specimenType)
...
combined <- rbind.fill(combined, mut_table_final_TCGA_sub)
...
# =========================================================================================================================================================
# seems like there are rare mutations in the stanford samples that are not recurrent in the other studdies. For now, filter to only mutations found in TCGA
# =========================================================================================================================================================
...
# Combine dataframes ####
# =======================
combined_2 <- rbind.fill(combined, temp_final_flt3)
...
joined_muts_clinical <- joined_muts_clinical[,c(1:5,10:12)]
...
joined_muts_clinical <- select(joined_muts_clinical, SAMPLE_NAME, GENE, VARIANT_TYPE, VAF, TypeAML, AA_CHANGE)
colnames(joined_muts_clinical)[1:6] <- c("labId", "symbol", "variant_type", "VAF", "Subset", "amino_acid_change")
...
# Combine dataframes ####
# =======================
final_data_matrix_sub <- rbind.fill(combined_2, joined_muts_clinical)
...
TCGA_survival2 <- select(TCGA_survival, PATIENT_ID, OS_STATUS, OS_MONTHS, SEX, AGE, CYTOGENETIC_CODE_OTHER, RISK_MOLECULAR, BM_BLAST_PERCENTAGE, PB_BLAST_PERCENTAGE, WBC)
# need to convert tcga data from months to days
TCGA_survival2$OS_MONTHS <- (TCGA_survival2$OS_MONTHS*30)

# Beat AML (Tyner et al.)
BeatAML_survival2 <- select(BeatAML_sample_data_types,  LabId, PatientId, vitalStatus, overallSurvival, consensus_sex, ageAtDiagnosis, Karyotype, ELN2017, `%.Blasts.in.BM`, `%.Blasts.in.PB`, WBC.Count, Hemoglobin, LDH, Platelet.Count)
...
Stanford_survival2 <- select(Stanford_survival, `Patient SU Number`, `Overall Survival (Event = Death)`, `Duration from diagnosis to D1`, Gender, `Age at Diagnosis`, Cytogenetics, 15)


# Papaemmanuil et al.
Multistage_survival2 <- select(Multistage_survival, PDID, Status, OS, gender, AOD, complex, M_Risk, BM_Blasts, PB_Blasts, wbc, HB, LDH, platelet)
...
colnames(Stanford_survival2) <- c("Sample", "Censor", "Time_to_OS", "Sex", "Age", "Cytogenetics", "Risk")
...
# =========================================
# Other cohorts with sequenced  AML samples
# =========================================
...
wang <- wang[-1,]
wang <- select(wang, `Sample ID`, Gene, `Amino acid`)
wang$Subset <- "de_novo"
...
for(i in 1:nrow(Au)){
  if(Au$symbol[i] == "FLT3_ITD" | Au$symbol[i] == "FLT3_TKD"){
    Au$symbol[i] <- "FLT3"
  }
}
...
welch_clinical <- na.omit(select(welch_clinical, 1,3,5:9,18:21))
...
# rm(garg_clin_1, garg_clin_2, garg_mut_1, garg_mut_2, garg_mut, garg_clin)
...
# select only diagnostic mutations
...
# subset to useful columns
garg_data <- select(garg_data, `Sample ID`, `Annotated Gene`, `Mutation Type`, `Amino Acid Change`, 21, `Dead/ Alive`, Sex, Age, `Cytogenetic (DX)`)
...
# subset to useful columns
flt3_add <- select(flt3_add, `Sample ID`, `Dead/ Alive`, Sex, Age, `Cytogenetic (DX)`, 14)
...
# column names
huet <- huet %>% rename(Sample = 'Patient ID', Subset = "de novo, secondary or therapy related AML", Risk = "ELN 2017 category",  WBC = 'Absolute leucocyte count (109/L)', LDH = "LDH (UI/L)", Hemoglobin = "Hemoglobin (gr/L)", Platelet = "Platelet count (109/L)", BM_blast_percent = "Blast cells in bone marrow (%)", PB_blast_percent =  "Blast cells in peripheral blood (%)", Time_to_OS = "Time to final outcome (months)", Censor = "Final outcome")
...
# download.file("https://static-content.springer.com/esm/art%3A10.1038%2Fncomms12475/MediaObjects/41467_2016_BFncomms12475_MOESM1148_ESM.xlsx", destfile = "~/Desktop/MetaAML_results/Data/41467_2016_BFncomms12475_MOESM1148_ESM.xlsx")
# 
# download.file("https://static-content.springer.com/esm/art%3A10.1038%2Fncomms12475/MediaObjects/41467_2016_BFncomms12475_MOESM1148_ESM.xlsx", destfile = "~/Desktop/MetaAML_results/Data/41467_2016_BFncomms12475_MOESM1149_ESM.xlsx")
...
hirsch_clinical <- hirsch_clinical %>% rename(Sample = UPN,  Sex = Gender, WBC = 'WBC (x 10 9/L)', BM_blast_percent = "% Blast cells in BM", Time_to_OS = "time from diagnosis (days)", Censor = "alive at last follow-up")
...
azizi_data <- select(azizi_data, Case, Gene, DX_VAF, REL_VAF, AA_Change, Overall_Survival, Gender, Age, DX_Karyotype, ELN, AML_Type, DX_Blasts, REL_Blasts, Cohort)
...
# remove any mutations that aren't called in at least two patients (to remove batch effects and noise from sequencing)
sub=subset(final_data_matrix, final_data_matrix$Cohort == "Stanford")
sub=unique(sub$symbol)

final_data_matrix = subset(final_data_matrix, final_data_matrix$symbol %in% sub)
...
# subset final matrix to only include mutatinos called in at least two patients
...
# for my boi Ravi, make the stanford data Majeti lab annotated
for(i in 1:nrow(final_data_matrix)){
...
Tumor_supressors <- list("TP53", "PHF6", "WT1")
...
for(i in 1:nrow(final_data_matrix)){
  ...
  if(final_data_matrix$Gene[i] %in% Tumor_supressors){
    final_data_matrix$mutation_category[i] <- "Tumor supressors"
  }
}


# save the final data frame as an .RData file
save(final_data_matrix,  file = "~/Desktop/MetaAML_results/final_data_matrix.RData")
write.csv(final_data_matrix,  file = "~/Desktop/MetaAML_results/final_data_matrix.csv", row.names = F)
